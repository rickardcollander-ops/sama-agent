...
async def create_comparison_page_pr(
    competitor: str,
    content: str,
    repo_owner: str = "rickardcollander-ops",
    repo_name: str = "successifier-homepage"
) -> Dict[str, Any]:
    """Create a competitor comparison page as a GitHub Pull Request."""
    from datetime import datetime
    ...
    comp_slug = _re_local.sub(r'[^a-z0-9]+', '-', competitor.lower()).strip('-')
    branch_name = f"content/vs-{comp_slug}-{datetime.utcnow().strftime('%Y%m%d%H%M')}"
    ...
    file_path = f"app/vs/{comp_slug}/page.tsx"
    sections = _markdown_to_jsx_sections(content, competitor)
...
"""
    ...
    file_result = await create_or_update_file(
        ...
        file_path=file_path,
        content=page_content,
        commit_message=f"content: add comparison page — Successifier vs {competitor.title()}",
        branch=branch_name,
        github_token=token
    )
    ...
    # 4. Create PR
    pr_body = f"""## Comparison Page — AI Generated

**Successifier vs {competitor.title()}**
**Target URL:** `/vs/{comp_slug}`

---
*Generated by SAMA Content Agent. Review content before merging.*
"""

    pr_result = await create_pull_request(
        repo_owner=repo_owner,
        repo_name=repo_name,
        title=f"Comparison: Successifier vs {competitor.title()}",
        body=pr_body,
